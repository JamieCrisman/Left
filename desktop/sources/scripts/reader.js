class Reader {
  ...
  start () {
    ...
    this.segment.text = left.textarea_el.value
      .substr(this.segment.from, this.segment.to - this.segment.from)
      .replace(/\n/g, ' ')
  ...
  alert (t) {
    setTimeout(t => {
      left.stats.el.innerHTML = `<b>Reader</b> Select some text before starting the reader.`
    }, 400)
  }

  run () {
    if (left.reader.queue.length === 0) {
      left.reader.stop()
      return
    }
    ...
    html +=
      ...
      orp.key.trim() +
      "</span><span class='fm'>" +
      (word.length > 1 ? orp.after : '').trim() +
      '</span>'
    html +=
      "<span style='float:right'>" +
      left.reader.queue.length +
      'W ' +
      parseInt((left.reader.queue.length * 175) / 1000) +
      'S ' +
      parseInt(
        (left.reader.index / parseFloat(left.reader.queue.length + left.reader.index)) * 100
      ) +
      '% ' +
      parseInt((1000 / left.reader.speed) * 60) +
      'W/M</span>'
